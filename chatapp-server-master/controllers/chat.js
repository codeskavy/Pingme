    ...
    return {
      ...
      avatar: groupChat
        ? members
            .filter(Boolean)
            .slice(0, 3)
            .map(({ avatar }) => avatar?.url || "") // Filtered null members and added optional chaining
        : [otherMember?.avatar?.url || ""], // Added optional chaining
      name: groupChat ? name : otherMember?.name, // Added optional chaining
      members: members.filter(Boolean).reduce((prev, curr) => {
        // Filtered null members
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  const uniqueMembers = allNewMembers
    .filter((i) => i && !chat.members.includes(i._id.toString())) // Filter out null/undefined users
    .map((i) => i._id)
  ...
  const allUsersName = allNewMembers
    .filter(Boolean)
    .map((i) => i.name)
    .join(", ") // Filter out null/undefined users
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members, {
    message: `${userThatWillBeRemoved?.name || "A user"} has been removed from the group`, // Added optional chaining
    chatId,
  })
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members, {
    chatId,
    message: `User ${user?.name || "A user"} has left the group`, // Added optional chaining
  })
...
const getChatDetails = TryCatch(async (req, res, next) => {
  if (req.query.populate === "true") {
    ...
    chat.members = chat.members.filter(Boolean).map((member) => ({
      ...
      _id: member._id,
      name: member.name,
      avatar: member.avatar?.url || "", // Added optional chaining
    }))
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  await Promise.all([deletFilesFromCloudinary(public_ids), chat.deleteOne(), Message.deleteMany({ chat: chatId })])
